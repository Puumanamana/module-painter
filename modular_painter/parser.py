...
def parse_args():
    ...
    if not (args.parents or args.children):
        args.rename = not (args.dataset == "fromageries")
        populations = sorted(Path(TEST_DIR, args.dataset).glob("*.fasta"))
        args.children = [populations.pop()]
        args.parents = populations
